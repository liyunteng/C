 ...
 ssize_t msg_send(int fd, struct msg *mptr)
 {
	 return (write(fd, mptr, MSGHDRSIZE + mptr->msg_len));
 }

 ssize_t msg_recv(int fd, struct msg *mptr)
 {
	 size_t	len;
	 ssize_t n;
	 ...
	 if ((n = read(fd, mptr, MSGHDRSIZE)) == 0) {
		 ...
		 err_quit("message header: expected %d, get %d",
			  MSGHDRSIZE, n);
	 }

	 if ((len = mptr->msg_len)  > 0) {
		 if ((n = read(fd, mptr->msg_data, len)) != len)
			 err_quit("message data: expected %d, get %d",
				  len, n);
	 }
	 return (len);
