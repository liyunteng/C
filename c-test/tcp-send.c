...
main(int argc, char *argv[])
{
    ...
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPIDLE, &keep_idel,
                   sizeof(keep_idel))
        < 0) {
        perror("keep idel");
        return -1;
    }
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPINTVL, &keep_interval,
                   sizeof(keep_interval))
        < 0) {
        perror("keep interval");
        return -1;
    }
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPCNT, &keep_count,
                   sizeof(keep_count))
        < 0) {
        perror("keep count");
        return -1;
    ...
    tv.tv_sec  = 0;
    tv.tv_usec = 0;
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_USER_TIMEOUT, &tv, sizeof(tv))
        < 0) {
        perror("setsockopt");
        return -1;
    }
