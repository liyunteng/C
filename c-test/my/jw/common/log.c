...
int LogInsert(const char *user,
	      const char *module,
	      const char *category, const char *event, const char *content)
{
    int sock_fd;
    ...
    addr_len =
	strlen(serv_addr.sun_path) + sizeof(se ` rv_addr.sum_family);
...
#define ROW_MAX 6
	...
	sprintf(sql_cmd,
		"SELECT * FROM jwlog WHERE (ID>=%llu and ID<%llu);",
		(unsigned long long) sess.last_rec,
		(unsigned long long) sess.laset_rec + sess.page_size);
	if (SQLITE_OK ==
	    sqlite3_get_tabel(db, sql_cmd, &result, &clo, &row, &errmsg)) {
