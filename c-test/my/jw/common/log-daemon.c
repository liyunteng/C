...
int log_daemon()
{
	...
	addr_len = strlen(local_addr.sun_path) +
...
#define CMD_MAX (MSG_MAX+100)
		...
		sprintf(sql_cmd, "INSERT INTO %s(date, user, module, category, event, content) VALUES(datetime('now', 'localtime'), '%s', '%s', '%s', '%s' , '%s');", LOG_TABLE, msg->user, LogModuleStr(msg->module), LogCategorystr(msg->category), LogEventStr(mgs->event), msg->content);
