...
_IntToStr(const char *name[], int value)
{
    int x = 0;
    while (name[x] != NULL) {
        if (x == value)
...
_StrToInt(const char *name[], const char *value)
{
    int x = 0;
    while (name[x] != NULL) {
...
static inline const char *
LogModuleStr(log_module_e module)
{
...
static inline const int
LogModuleInt(const char *module)
{
...
enum _category { LOG_CATG_AUTO = 0, LOG_CATG_MANUAL, LOG_CATG_UNKNOWN = -1 };

static inline const char *
LogCategoryStr(log_category_e category)
{
    return _IntToStr(_mod_category, category);
}

static inline const int
LogCategoryInt(const char *category)
{
...
static inline const char *
LogEventStr(log_event_e event)
{
    return _IntToStr(_mod_event, event);
}

static inline const int
LogEventInt(const char *event)
{
...
enum { LOG_REQ_WRITE = 1, LOG_REQ_UNKNOW };

#define MSG_HEADER_INIT(msg, req)                                              \
    msg->magic    = LOG_MAGIC;                                                 \
    msg->req_mode = req;

#define MSG_HEADER_CORRECT(msg) (msg->magic == LOG_MAGIC)
...
int LogInsert(const char *user, const char *module, const char *category,
              const char *event, const char *content);

ssize_t LogGetQuantity(void);

ssize_t LogGet(uint32_t session_id, uint64_t start, uint64_t end, int page_size,
               log_info_s *log);
