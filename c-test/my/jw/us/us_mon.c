...
static void
us_mon_do_action(const char *path, const char *dev, const char *act)
{
    struct list *p;
...
us_mon_init(void)
{
    ...
    us_mon.udev = udev;
    us_mon.mon  = mon;
    ev_io_init(&us_mon.udev_io, udev_io_cb, udev_monitor_get_fd(mon), EV_READ);
...
int
us_mon_register_notifier(struct mon_node *node)
{
...
void
us_mon_unregister_notifier(struct mon_node *node)
{
    list_del(&node->list);
}
