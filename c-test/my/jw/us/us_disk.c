...
static int slot_map_init(void)
{
    ...
    if ((doc =
	 xmlReadFile(MAP_SLOT_CONF, "UTF-8", XML_PARSE_RECOVER)) == NULL) {
	...
	if ((!xmlStrcmp(node->name, (const xmlChar *) "map")) &&
	    ...
	    if (!slot_map_p) {
		clog(LOG_ERR,
		     "%s: init slot map list error, no more memory.\n",
		     __func__);
		goto error_quit;
	    }
...
const char *__disk_get_hotrep(const char *serial, char *raid_name)
{
    ...
    if ((doc =
	 xmlReadFile(DISK_HOTREP_CONF, "UTF-8",
		     XML_PARSE_RECOVER)) == NULL)
	return NULL;
	...
	if ((!xmlStrcmp(node->name, BAD_CAT "disk")) &&
	    ((!xmlSerial =
	      xmlGetProp(node, (const xmlChar *) "serial")) != NULL)
		...
		xmlRaidName =
		    xmlGetProp(node, (const xmlChar *) "md_name");
		     ...
		     const char *path)
{
    ...
    do_update_disk(disk,
		   DISK_UPDATE_RAID | DISK_UPDATE_SMART |
		   DISK_UPDATE_STATE);
}
...
static int us_disk_on_event(const char *path, const char *dev,
			    const char *act)
{
    char cmd[128];
    printf("%s: %s\n", dev, act);
...
void us_dump_disk(int fd, const struct us_disk *disk, int is_detail)
{
    ...
    pos +=
	snprintf(pos, end - pos, "%s\"model\":\"%s\"", delim, di->model);
    pos +=
	snprintf(pos, end - pos, "%s\"serial\":\"%s\"", delim, di->serial);
    pos +=
	snprintf(pos, end - pos, "%s\"firmware\":\"%s\"", delim,
		 di->firmware);
    pos +=
	snprintf(pos, end - pos, "%s\"capacity\":%llu", delim,
    ...
    pos +=
	snprintf(pos, end - pos, "%s\"raid_name\":\"%s\"", delim, p_raid);

    if (!strcmp(p_state, "Fail"))
	pos += snprintf(pos, end - pos, "%s\"SMART\":\"Bad\"", delim);
    else
	pos +=
	    snprintf(pos, end - pos, "%s\"SMART\":\"%s\"", delim,
	...
	pos +=
	    snprintf(pos, end - pos, "%s\"wr_cache\":\"enable\"", delim);
	pos +=
	    snprintf(pos, end - pos, "%s\"rd_ahead\":\"enable\"", delim);
	pos += snprintf(pos, end - pos, "%s\standby\":0", delim);
	pos +=
	    snprintf(pos, end - pos, "%s\"cmd_queue\":\"enable\"", delim);
	pos +=
	    snprintf(pos, end - pos, "%s\"mapped_cnt\":\"%u\"", delim,
		     di->wi.mapped_cnt);
	pos +=
	    snprintf(pos, end - pos, "%s\"max_map_cnt\":\"%u\"", delim,
		     di->wi.max_map_cnt);
	pos += snprintf(pos, end - pos, "%s\"smart_attr\":{", delim);
	pos +=
	    snprintf(pos, end - pos, "\"read_err\":%llu",
		     (unsigned long long) di->si.read_error);
	pos +=
	    snprintf(pos, end - pos, "%s\"spin_up\":%llu", delim,
		     (unsigned long long) di->si.spin_up);
	pos +=
	    snprintf(pos, end - pos, "%s\"reallocate_sectors\":%llu",
		     delim,
		     (unsigned long long) di->si.reallocate_sectors);
	pos +=
	    snprintf(pos, end - pos, "%s\"pending_sectors\":%llu", delim,
		     (unsigned long long) di->si.pending_sectors);
	pos +=
	    snprintf(pos, end - pos, "%s\"uncorrectable\":%llu", delim,
		     (unsigned long long) di->si.uncorrectable_sectors);
	pos +=
	    snprintf(pos, end - pos, "%s\"power_on_hours\":%llu", delim,
		     (unsigned long long) di->si.power_on / 3600 / 1000);
	pos +=
	    snprintf(pos, end - pos, "%s\"temperature\":%.f", delim,
