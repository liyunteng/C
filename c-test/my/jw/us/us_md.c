...
static int
find_free_slot(struct us_disk_pool *dp)
{
    int i;
...
static void
do_update_disk(struct us_disk *disk, const char *dev)
{
...
static int
us_disk_on_event(const char *path, const char *dev, int act)
{
    if (!is_sata_sas(path))
        return MA_NONE;
    if (act == MA_ADD)
