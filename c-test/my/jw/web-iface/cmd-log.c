...
{
	printf(_T("\nlog\n\n"));
	printf(_T("Usage: --insert [--user <name>] --module <name> --category <auto|manul> --event <evnet_info> --content <content_text>\n"));
	printf(_T("		module:Web, Disk, VG, UDV, iSCSI, NAX, SysConf\n"));
...
void log_print(log_info_s *info, size_t num)
{
	...
	for (i = 0; i < num; i++) {
		if (i > 0)
			printf(",");
		printf("\n\t\t{\"datetime\":\"%s\", \"module\":\"%s\", \"category\":\"%s\", \"event\":\"%s\", \"content\":\"%s\"", info[i].datetime, info[i].module, info[i].category, info[i].event, info[i].content);
	}
	if (num > 0)
		printf("\n\t]\n");
	else
