...
int
_dir_max_file_num(const char *dir_path)
{
    int max = -1;
...
tmpfs_msg_remove_oldest(const char *level)
{
    ...
    _DIR_LIST_END if ((oldest_file[0] != '\0') && !unlink(oldest_file))
    {
        return oldest_file;
    }
...
ssize_t
tmpfs_msg_sorted_link(const char *file)
{
    char link[PATH_MAX];
...
ssize_t
tmpfs_msg_sorted_unlink(const char *file)
{
    ssize_t ret = -1;
