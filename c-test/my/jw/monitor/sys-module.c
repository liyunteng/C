...
bool sys_module_add(const char *name)
{
	...
	syslog(LOG_NOTICE, "sys_module_add('%s'): add fail, module memory alloc faile",
			name);
...
bool sys_module_event_add(const char *name, sys_event_conf_t *event)
{
	...
	if ( (module = sys_module_get(name)) == NULL) {
		syslog(LOG_NOTICE, "sys_module_event_add('%s', '%s'): module not exist!",
				name, event->event);
		return false;
	}
...
sys_event_conf_t *sys_module_event_get(const char *name, const char *event)
{
	...
	if ( (module=sys_module_get(name)) == NULL) {
		syslog(LOG_NOTICE, "sys_module_event_get('%s', '%s'): module not exist!",
				name, event);
