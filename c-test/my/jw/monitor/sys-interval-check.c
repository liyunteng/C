...
int _value_check_error(sys_capture_t * cap, char *msg)
{
	...
	sprintf(msg, "当前值 %d, 未达到最低值 %d", value,
		cap->min_thr);
	return VAL_ERROR;
    } else if (value > cap->max_thr) {
	sprintf(msg, "当前值 %d, 已超过最高值 %d", value,
		cap->max_thr);
	return VAL_ERROR;
    }
...
void _capture(sys_capture_t * cap)
{
	...
	if (cap->_error != VAL_ERROR) {
	    ...
	    sysmon_event("self_run", "env_exception_raise", cap->name,
			 msg);
	    sprintf(log_msg, "监控模块%s告警: %s",
		    MOD_NAME(cap->name), msg);
	    LogInsert(NULL, "SysMon", "Auto", "Error", log_msg);
	...
	if (cap->_error != VAL_NORMAL) {
		...
		sysmon_event("self_run", "env_exception_backout",
			     cap->name, "good");
	    cap->_error = VAL_NORMAL;
	    sprintf(log_msg, "监控模块%s告警解除",
		    MOD_NAME(cap->name));
	    LogInsert(NULL, "SysMon", "Auto", "Error", log_msg);
