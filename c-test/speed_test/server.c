...
void *thread(void * a)
{
	...
	if (t == 0)
        ...
        fprintf(stderr, "%s:%d Upload size: %llu nrecv: %llu time: %llu speed: %llu kB/s\n",
                inet_ntoa(arg.addr.sin_addr), ntohs(arg.addr.sin_port), count, nrecv, t, count / t * 1000 / 1024);
	...
	if (nsent == 0)
        ...
        fprintf(stderr, "%s:%d Download size: %llu  nsent %llu time: %llu speed: %llu kB/s\n",
                inet_ntoa(arg.addr.sin_addr), ntohs(arg.addr.sin_port), count, nsent, t, count / t * 1000 / 1024);
