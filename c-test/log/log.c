...
static const char *const LOGLEVELSTR[] = {
    "NONE", "EMERG", "ALERT", "FATAL", "ERROR", "WARN", "NOTICE", "INFO", "DEBUG",
};

static inline void
makebak(const char *filename)
{
    int  bakcnt;
...
loger *
log_create(const char *file, LOGLEVEL level)
{
    loger *handler = NULL;
...
void
vlog(loger *handle, LOGLEVEL level, const char *file, size_t filelen, const char *function,
     size_t functionlen, long line, const char *format, va_list args)
{
    if (handle == NULL)
        return;
    ...
    idx = snprintf(buf, sizeof(buf), "%s [%-5.5s]  %s %s():%ld ", timebuf, LOGLEVELSTR[level], file,
                   function, line);
#endif
    ...
    if (stat(handle->lfp->logfile, &st) == 0) {
        if (MAX_BAK > 0 && (st.st_size + len >= MAX_FILESIZE)) {
            /* fprintf(stderr, "size: %lu, len: %lu\n", (unsigned long)st.st_size, (unsigned
             * long)len); */
            ...
            /*
             * close(handle->lfp->fd);
             * handle->lfp->fd = open(handle->lfp->logfile, O_CREAT | O_APPEND | O_WRONLY,
             * FILEPERM);
             */
        }
    }
...
void
mlog(loger *handle, LOGLEVEL level, const char *file, size_t filelen, const char *function,
     size_t functionlen, long line, const char *format, ...)
{
...
int
slog_init(const char *file, LOGLEVEL level)
{
...
void
slog(LOGLEVEL level, const char *file, size_t filelen, const char *function, size_t functionlen,
    ...
    for (i = 0; i < MAX_OPENFILE; i++) {
        if (lfps[i].fp != NULL) {
            fprintf(
                ...
                "%llu\nemerg: %llu\n"
                "unhandle: %llu\n\n",
                lfps[i].logfile, (unsigned long long)lfps[i].success_count,
                (unsigned long long)lfps[i].success_byte, (unsigned long)lfps[i].fprintf_fail_count,
                (unsigned long)lfps[i].makebak_count, (unsigned long long)lfps[i].debug_count,
                (unsigned long long)lfps[i].info_count, (unsigned long long)lfps[i].warning_count,
                (unsigned long long)lfps[i].error_count, (unsigned long long)lfps[i].fatal_count,
                (unsigned long long)lfps[i].alert_count, (unsigned long long)lfps[i].emerg_count,
