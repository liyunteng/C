					    ...
					    struct buffer_head **bh)
{
	...
	if (block_group >= ngroups) {
		ext4_error(sb, "block_group >= group_count - block_group = %u," "groups_count = %u", block_group, ngroups);
		return NULL;
	}
					    ...
					    struct buffer_head *bh)
{
	...
	offset = blk - group_first_block;
	next_zero_bit = ext4_find_next_zero_bit(bh->b_data					offset + EXT4_SB(sb)->s_itb_per_group, offset);
